...
> [!NOTE]
> [はじめに](intro.md#%E3%83%87%E3%83%97%E3%83%AD%E3%82%A4%E3%81%95%E3%82%8C%E3%82%8B-azure-%E3%83%AA%E3%82%BD%E3%83%BC%E3%82%B9%E3%81%A8-ai-%E3%83%A2%E3%83%87%E3%83%AB) に記載されている情報を参考に、Bicep を使用せずに Azure Portal や Azure CLI を使用して手動で作成しても構いませんが、リソースの作成先のリージョンは Azure OpenAI Service だけが `East US` で(※)、他のリソースは `Japan West` となるのでご注意ください。

> (※) Microsoft Foundry が提供する AI モデルはすべてのリージョンで同じものがホストされているわけではなく、リージョン毎に提供されている AI モデルが異なる可能性があります。そのためアプリケーションをホストするリージョンと、AI モデルをデプロイ可能であるリージョンが異なる場合が多くあります。今回のハンズオンではこのような現実的なシナリオを想定し、AI モデルは `East US` リージョンにデプロイし、アプリケーションは `Japan West` リージョンにホストする構成でリソースを作成します。

>[!IMPORTANT]
>もし、なんらかの理由で他のリージョンを使用する必要がある場合は、ファイル **prep-az-resource.bicep** の内容を以下のように変更し他のリージョンでリソースを作成してください。
> ```bicep
...
3. Cloud Shell 画面で、以下のコマンドを実行してリソースグループ `AOAI-AppEnv-handson` を作成します
   ```bash
   az group create --name AOAI-AppEnv-handson --location "Japan West"
   ```
...
6. デプロイが完了したら Azure ポータル画面で `AOAI-AppEnv-handson` リソースグループを開き、以下のリソースが作成されていることを確認します。
   ...
   もし、コマンドが正常に実行されたにもかかわらず、なんらかのリソースが作成されていない場合は、Azure ポータルの画面を更新してみてください。それでもリソースが表示されない場合は、Azure CLI で以下のコマンドを使用して確認してください。

    ```bash
    az resource list --resource-group AOAI-AppEnv-handson --output table
    ```

7. デプロイされた Azure Storage Account リソース `st(ランダムな値)`を開き、\[**BLOB コンテナー**\] メニューをクリックして、`rag-data-store` という名前の BLOB コンテナーが作成されていることを確認します。
   
    ![Azure Blob Container](images/check_storageBlob.png) 

8. デプロイされた AI モデルを確認します。 Azure OpenAI サービスのリソース `aoai-(ランダムな値)` を開き、\[**概要**\] メニュー画面内にある \[**探して展開する**\] ボックス内の \[**Foundry ポータルの詳細**\]ボタンをクリックします

    ![Foundry ポータルの詳細 ボタン](./images/DeployModel_OpenAIStudio.png)

9.  Azure AI Foundry が開かれるので、画面左のメニューバーから \[**デプロイ**\] をクリックします

    ![Azure AI Foundry デプロイメニュー](./images/AOAIStudio_menu_Deploy.png)

10. **gpt-5-chat**、**text-embedding-3-small** の 2 つのモデルがリストされることを確認します。もし、リストされていない場合は、画面上部の \[**モデルのデプロイ**\] ボタンをクリックして、モデルを選択し、デプロイを行ってください。
