...
>[!IMPORTANT]
>もし、`Japan East` リージョンでのデプロイでキャパシティ不足のエラーが発生した場合は、ファイル **prep-az-resource.bicep** の内容を以下のように変更し`Japan West` リージョンでリソースを作成してください。
> ```bicep
>@description('リソースをデプロイするリージョン（通常リソース）')
>param location string = 'Japan West'
>```
>また移行の作業手順内容は、`Japan East` を `Japan West` に置き換えて実施してください。
