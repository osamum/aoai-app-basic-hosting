...
可用性ゾーンの利用は Azure App Service の運用環境デプロイに関する[推奨事項](https://learn.microsoft.com/ja-jp/azure/architecture/web-apps/app-service/architectures/baseline-zone-redundant)にもなっています。
...
可用性ゾーンは [Azure のさまざまなサービスでサポート](https://learn.microsoft.com/ja-jp/azure/reliability/availability-zones-service-support)されていますが、この演習ではデプロイしたリソース Azure App Service と Azure Storage、Azure AI Search の可用性ゾーンの設定を行います(※)。

(※) 公式ドキュメントで詳細な手順が提供されているものについては、手順のリンクを提供します。

## 🌐 App Service の可用性ゾーンの設定

App Service をゾーン冗長構成にすることで、単一のゾーン（データセンター）に障害が発生しても、他のゾーンに自動的にフェイルオーバーされ、サービスを継続できるようになります。
...
> また、データセンターの**キャパシティの関係で App Service プランの作成に失敗する場合は、この手順はスキップして次の演習に進んでください**。
...
5. \[**App Service プランの作成**\] 画面が表示されるので、以下の項目を入力します
    ...
    App Service プランのデプロイが開始されるので完了するまで待ちます。
...
ここまでの手順で、演習用の App Service のプランを変更し、可用性ゾーンを設定することができました。
...
* 可用性ゾーンを有効にして作成した新しいストレージ アカウントにデータを移行する
* 既存のストレージ アカウントを変換する
