...
2. 画面左にのメニューから`アクセス制御(IAM)`をクリックします。

    ![Container IAM](images/storage_iam.png) 

3. `追加`をクリックし、`ロールの割り当ての追加`をクリックします。  

    ![Assign Role](images/assign_role.png) 

4. 検索ボックスに`blob`と入力し、`ストレージBLOBデータ所有者`を選択後、`次へ`をクリックします。

    ![Assign Blob Owner](images/assign_blob_owner.png) 

5. アクセスの割り当て先に`ユーザー、グループ、またはサービスプリンシパル`を選択し、`+メンバーを選択する`をクリックします。

    ![Setting Select Member](images/setting_select_member.png) 

6. 表示されたメンバーのうち、自身のアカウントを選択します。この場合は、`System Administrator`を選択します。選択後、`選択`をクリックします。

    ![Select Member](images/select_member.png) 

7. 自身が選択したメンバーが表示されていることを確認し、`レビューと割り当て`をクリックします。

    ![Member Review](images/member_review.png) 

8. `概要`をクリックします。

    ![Back to Desc](images/back_to_desc.png) 
...
10. Azure Storage Account の画面で、\[設定\]-\[**構成**\] メニューをクリックします。遷移した画面で \[**Azure portal で Microsoft Entra 認可を既定にする**\] の設定を **有効** にし、画面上部の \[**保存**\] ボタンをクリックします。
...
1. 作業中の Azure AI Search リソース `search(ランダムな値)` の \[概要\] 画面を開き、画面上部の \[**データのインポート(新規)**\] ボタンをクリックします
