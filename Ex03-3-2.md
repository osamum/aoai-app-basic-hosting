...
おおまかな手順としては、まず最初に Log Analytics ワークスペースを作成し、次に各 Azure リソースに対して Log Analytics を有効にします。そして、各リソースのログをクエリーしてエラーや監視対象イベントの発生状況を確認します。

この演習で実施する作業は以下のとおりです。

1. [Log Analytics ワークスペースの作成](#1-log-analytics-%E3%83%AF%E3%83%BC%E3%82%AF%E3%82%B9%E3%83%9A%E3%83%BC%E3%82%B9%E3%81%AE%E4%BD%9C%E6%88%90)
2. [App Service への診断設定の追加](#2-app-service-%E3%81%B8%E3%81%AE%E8%A8%BA%E6%96%AD%E8%A8%AD%E5%AE%9A%E3%81%AE%E8%BF%BD%E5%8A%A0)
3. [OpenAI Service への診断設定の追加](#3-azure-openai-%E3%82%B5%E3%83%BC%E3%83%93%E3%82%B9%EE81%B8%E3%81%AE%E8%A8%BA%E6%96%AD%E8%A8%AD%E5%AE%9A%E3%81%AE%E8%BF%BD%E5%8A%A0)
4. [AI Search への診断設定の追加](#4-azure-ai-search-%E3%82%B5%E3%83%BC%E3%83%93%E3%82%B9%EE81%B8%E3%81%AE%E8%A8%BA%E6%96%AD%E8%A8%AD%E5%AE%9A%E3%81%AE%E8%BF%BD%E5%8A%A0)
5. [Storage アカウントへの診断設定の追加](#5-azure-storage-%E3%82%A2%E3%82%AB%E3%82%A6%E3%83%B3%E3%83%88%EE81%B8%E3%81%AE%E8%A8%BA%E6%96%AD%E8%A8%AD%E5%AE%9A%E3%81%AE%E8%BF%BD%E5%8A%A0)
...
Log Analytics ワークスペースを作成する具体的な手順は以下のとおりです。
...
2.  診断設定の追加の作成画面に遷移するので、同画面の各項目を以下のように設定します
    ...
    * **宛先の詳細**
    ...
    |Log Analytics ワークスペース| \[**AOAI-AppEnv-LogAnalytics (japanwest)**\] (※)|
...
3. App Service の画面左のメニューの \[**監視**\] グループにある \[**ログ**\] をクリックします
    ...
    **もし表示されていない場合**は、検索ボックス右側の \[…\] (三点リーダー)ボタンをクリックし、表示されたメニューから \[**データがないテーブルの表示**\]をクリックします。

    ![AppService のログ テーブル](images/Azure_log_table.png)

4.  \[**テーブル**\] タブのツリービュー \[**App Services**\] に表示されている `AppServiceHTTPLogs` を**ダブルクリック**するか、マウスをホバーして表示される \[**実行**\] ボタンをクリックします
   
    ![AppServiceHTTPLogs のクエリ実行](images/AppSrv_log_queryHttpLogAll.png)
...
5. リクエストを HTTP500 エラーでフィルターします
    ...
    \[新しいクエリー\] タブの上部にあるフィルターの \[追加\] ボタンをクリックして、\[**CsStatus**\]をクリックします

    ![フィルターの追加](images/AppSrv_log_queryAddFilter.png)

    
    遷移した画面で \[**500**\] のチェックボックスにチェックし、\[**適用**\] ボタンをクリックします

    ![HTTP500 エラーのフィルタリング](images/AppSrv_log_queryStatus500.png)

    \[CsStatus\] フィールドの値が **500** のログのみが表示されることを確認します

    ![HTTP500 エラーの一覧](images/AppSrv_log_queryAllErrorResult.png)

    \[新しいクエリー \] タブの右端にあるドロップダウンボックスから \[**KQLモード**\] を選択し、クエリーエディターを開きます。

    ![KQLモードの選択](images/AppSrv_log_KQL_mode.png)


    表示されたクエリーエディターの内容が以下のようになっていること確認します

    ```
    AppServiceHTTPLogs
    |   where ScStatus in (500)
    ```

    なお、クエリーエディターの内容は、Kusto クエリー言語 (KQL) で記述されており、直接書き換えて画面上部の \[**実行**\] ボタンをクリックすることで、クエリーの内容を変更して実行することができます。

6. HTTP500 (HTTP Server Error) をフィルターするクエリーを保存します

    画面左上部の \[**保存**\] - \[**クエリとして保存**\] をクリックすると、画面の右側に \[**クエリとして保存**\] ブレードが表示されるので、同ブレードの各項目を以下のように設定します。
...
2. クエリーエディタの画面に戻るので \[**クエリ**\] タブをアクティブにし、検索ボックスに `HTTPServerError` と入力してキーボードの \[Enter\] キーを押下ます
...
![AppServiceConsoleLogs のクエリ結果](images/AppSrv_log_queryConsoleLogAllResult.png)
...
2.  診断設定の追加の作成画面に遷移するので、同画面の各項目を以下のように設定します
    ...
    * **宛先の詳細**
    ...
    (※) 前の手順で作成した Log Analytics ワークスペースを選択します。ただし、もし、以降の手順で `AzureDiagnostics` テーブルにログが表示されない場合は、Log Analytics ワークスペースを **East US** のリージョンで新規で作成し、そちらを選択してください。
...
4. \[**AOAI-AppEnv-LogAnalytics**\] の画面が表示されるので、同画面左のメニューから \[**ログ**\] をクリックします
    ...
    \[**ログ**\] 画面の \[**新しいクエリ 1**\] タブが表示されるので、同タブ内の \[**テーブル**\] タブのツリービュー \[**Cognitive Services**\] に `AzureDiagnostics` が表示されていることを確認します。


    **もし表示されていない場合**は、検索ボックス右側の \[…\] (三点リーダー)ボタンをクリックし、表示されたメニューから \[**データがないテーブルの表示**\]をクリックします。

    ![](images/Azure_aoailog_table.png)

     \[**テーブル**\] タブのツリービュー \[**Cognitive Services**\] に表示されている `AzureDiagnostics` を**ダブルクリック**するか、マウスをホバーして表示される \[**実行**\] ボタンをクリックします

    ![AzureDiagnostics テーブルの表示](images/AOAI_log_daiagLogAll.png)
...
5. クエリー入力画面の \[**実行**\] ボタンをクリックし、ログの一覧が表示されることを確認します。
    ...
    現在のクエリーは \[**AzureDiagnostics**\] テーブルのすべてのログの一覧を表示しているので、フィルターを追加します。\[新しいクエリー \] タブの右端にあるドロップダウンボックスから \[**KQLモード**\] を選択し、クエリーエディターを開きます。

    ![KQLモードの選択](images/AOAI_log_KQL_mode.png)
    
    クエリーエディターの内容を以下のようにクエリーを変更して、演習用アプリケーションが使用している Azure OpenAI サービスの要求と応答のログのみを表示するようにします。

    なお、クエリー文中の `Resource` の値は、演習用ボットアプリケーションで使用している Azure OpenAI サービスのリソース名に置き換えてください。なお、リソース名は実際の名前とログに保存される名前とで大文字小文字が異なる場合があります。その場合はクエリー文中の `Resource` の値をログに保存されている名前に合わせてください。
...
## 4. Azure AI Search サービスへの診断設定の追加

Log Analytics ワークスペースに Azure AI Search サービスのログを送信する方法は、以下のドキュメントに詳しい手順が記載されているので、そちらを参照して実施してください。

* Azure AI 検索の診断ログを構成する - [**診断ログを有効にする**](https://learn.microsoft.com/ja-jp/azure/search/search-monitor-enable-logging#enable-diagnostic-logging)

上記手順の \[**診断設定を追加する**\] の設定では、\[**Log Analytics ワークスペースへの送信**\] のチェックボックスにチェックを入れ、このハンズオンで作成した Log Analytics ワークスペース `AOAI-AppEnv-LogAnalytics (japanwest)`を選択してください。

![AI Search への診断設定の追加](images/AISearch_New_DiagSetting.png)


なお、**診断設定** を追加した後、Azure AI Search サービスのログが Log Analytics ワークスペースに送信されるまでにある程度の時間がかかるので、しばらく時間をおいてから Log Analytics ワークスペースのログをクエリして確認してください。

また、演習用ボットアプリケーションに質問を投げかけて、ログが生成されるようにしてください。

クエリーされた Azure AI Search サービスのログの内容については、以下のリンクのドキュメントを参照してください。
...
## 5. Azure Storage アカウントへの診断設定の追加

Azure Storage アカウントへの診断設定の追加方法は、ここまでの演習でおこなってきた他の Azure リソースでの設定と同様ですが、Storage アカウントは異なる複数のストレージのサービスをサポートするため、その中のどのストレージのログを収集するかを選択する必要があります。
...
\[**ログ**\] メニューをクリックして遷移した画面の \[**テーブル**\] タブのツリービューに `Storage Accounts` が表示されていればログが利用可能になっています。

同タブのツリービュー \[**Storage Accounts**\] を展開し、\[**StorageBlobLogs**\] を**ダブルクリック**して、クエリー入力画面に `StorageBlobLogs` が入力されたことを確認します。

その後、クエリー入力画面の \[**実行**\] ボタンをクリックし、ログの一覧が表示されることを確認します。

![StorageBlobLogs クエリーの実行結果](images/Storage_log_queryHttpLogAllResult.png)

あとは、これまでの他の Azure リソースのログと同様に、クエリーを編集して必要なログをフィルタリングしたり、集計したりすることができます。

必要に応じて画面上部の \[**保存**\] - \[**クエリとして保存**\] をクリックして、クエリーを保存してください。

ログに含まれるフィールドの値については、以下のドキュメントを参照してください。

* Azure Blob Storage 監視データのリファレンス - [**リソースログ**](https://learn.microsoft.com/ja-jp/azure/storage/blobs/monitor-blob-storage-reference#resource-logs)

より詳しい Azure Blob Storage の監視については以下のドキュメントを参照してください。

* [**Azure Blob Storage の監視に関するベスト プラクティス**](https://learn.microsoft.com/ja-jp/azure/storage/blobs/blob-storage-monitoring-scenarios)
...
この演習ではハンズオンで使用している Azure リソースに対して Log Analytics にログを送信できるように構成し、アプリケーションのログをクエリーする手順を確認しました。

この演習では結果をフィルターする簡単なクエリーしか実行しませんでしたが、Log Analytics を使用すると、より複雑なクエリーを作成してログの分析や集計を行うことができます。具体的な方法については、以下のドキュメントを参照してください。

* [**チュートリアル: Log Analytics を使用する**](https://learn.microsoft.com/ja-jp/azure/azure-monitor/logs/log-analytics-tutorial)


この演習では、Azure リソースのログを Log Analytics ワークスペースに送信しましたが、それ以外でも Azure Storage アカウントやイベント ハブ、サードパーティーのサービスのログを送信することができますので必要に応じて設定してください。

![診断設定情報の送信先](images/other_source_DiagSetting.png)

診断設定情報の送信先については以下のドキュメントをご参照ください。

* Azure Monitor の診断設定 - [**Destinations**](https://learn.microsoft.com/ja-jp/azure/azure-monitor/platform/diagnostic-settings?tabs=portal#destinations)
...
👈　[演習 3-2-1 : メトリック アラートの設定](Ex03-3-1.md)
