...
4.  \[**インデックス JSON エディター**\] 画面が表示されるので、表示されている JSON をすべて選択してコピーし、メモ帳などのテキスト エディターに貼り付けます
    ...
    貼り付けた JSON から不要な `@odata.etag` プロパティを削除します。

    ```json
    "@odata.etag": "\"0x8DDB8EA2E7F667C\"",
    ```

    また、復旧の際に使用いる [API バージョン](https://learn.microsoft.com/ja-jp/rest/api/searchservice/search-service-api-versions) 2024-07-01 でサポートされない以下のプロパティも削除します。

    (※)まとまっては記述されていませんので、プロパティ名で検索を行いそれぞれの箇所で削除してください。
