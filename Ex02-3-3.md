...
* Azure AI Search のサービス (インスタンス)
* Azure AI Search サービス内のインデックス、スキルセット、データソース、インデクサー等の設定

この演習では、Azure AI Search サービスのバックアップとリストアを行い、インデックスを復旧する作業を行います。
...
Azure AI Search のサービス (インスタンス) には、ディスク上のイメージを取得するような、いわゆる一般的なバックアップ機能はありませんが、構造とインスタンス固有の情報は、ARM テンプレート、あるいは Bicep とパラメーターを保持しておけばこれらを使用して再デプロイを行うことで復旧が可能です。
...
ただし、管理者キーやアクセス制御 (IAM) の設定は含まれませんので、復旧後に再設定が必要です。

アプリケーションが Azure AI Search サービスへのアクセスに管理者キーを使用している場合は、復旧後に再取得する必要がありますが、マネージド ID を使用している場合は、Bicep で `roleAssignments` リソースを使用してアクセス制御 (IAM) の設定をテンプレート化しておくことができます。
...
復旧の際には、Azure AI Search の REST API を使用してインデックスを再作成します。

(※) インデックス内のベクトル フィールドの情報は、埋め込み(Embedding)モデルを使用して生成されており、埋め込みモデルは、アプリケーションがベクトル検索時に使用するものと同じものを使用する必要があります。よって、復旧する際、アプリケーションが使用している埋め込みモデルが変更されている場合は、インデックス内のベクトル フィールドの情報も再生成する必要があります。
...
バックアップした Bicep ファイルを使用して、Azure AI Search のインスタンスを再デプロイする手順は以下の通りです。
...
4. デプロイが完了したら Azure ポータル画面で `AOAI-AppEnv-handson` リソースグループを開き、Azure AI Search のインスタンスがバックアップしたものと同じ名前で作成されていることを確認します。

ここまでの手順で、Azure AI Search のインスタンスをバックアップから復旧できました。

### 補足

復旧された Azure AI Search のインスタンスではマネージド ID が自動的に作成されますが、マネージド ID 自体は別のものになっているため [Azure AI Search インデックスの作成](Ex01-3.md#azure-ai-search-%E3%82%A4%E3%83%B3%E3%83%87%E3%83%83%E3%82%AF%E3%82%B9%E3%81%AE%E4%BD%9C%E6%88%90) の作業で必要になる Azure Storage への `ストレージ BLOB データ閲覧者`ロールの [割り当て作業](Ex01-3.md#azure-ai-search-%E3%83%9E%E3%83%8D%E3%83%BC%E3%82%B8%E3%83%89-id-%E3%81%AE%E6%9C%89%E5%8A%B9%E5%8C%96%E3%81%A8%E3%82%A2%E3%82%AF%E3%82%BB%E3%82%B9%E8%A8%B1%E5%8F%AF)を再度行う必要があります。Azure Portal から手動で行っても構いませんが、以下の Bicep を使用してロール割り当てを行うこともできます。
...
とくに**管理者キーは、バックアップ時のものとは異なる**ので必ず取得し直してください。

バックアップしておいたインデックス定義ファイルを使用して、Azure AI Search のインデックスを復旧する手順は以下の通りです。
...
ただし、作成されたのはインデックスの構成のみで、インデックス内のベクトルデータは含まれていませんので、別途データを追加する必要があります。
...
この演習では、Azure AI Search のサービスとインデックスのバックアップとリストア方法を学びました。

最後の Azure App Service でホストされているアプリケーションの Azure AI Search 関連の環境変数を確認し、アプリケーションの RAG 機能が正しく動作することを確認してください。
...
また、非公式ではありますが Azure AI Search インデックスをバックアップして復元するコードサンプルも公開されているので、興味のある方は以下のリンクも参照してください。

* [**Back up and restore an Azure AI Search index**](https://learn.microsoft.com/en-us/samples/azure-samples/azure-search-dotnet-utilities/azure-search-backup-restore-index/)

<br>

## 次へ

👉　[**演習 2-4 : アプリケーションを利用するユーザーを制限する**](Ex02-4.md)

---

👈　[演習 2-3-1 : Azure Storage Blob のデータ保護機能とバックアップ](Ex02-3-2.md)
