# 演習 6-2 : セキュリティ設定の追加
 ...
 Application Gateway について、ベースラインアーキテクチャで推奨されているセキュリティ設定である **Web Application Firewall (WAF)** と **DDoS 保護** を追加します。

 ![](images/WAFandDDoSProtection_add.png)

 この演習では、以下の作業を行います。


 1. [Web Application Firewall ポリシーの作成と適用](#1-web-application-firewall-%E3%83%9D%E3%83%AA%E3%82%B7%E3%83%BC%E3%81%AE%E4%BD%9C%E6%88%90%E3%81%A8%E9%81%A9%E7%94%A8)
 2. [DDoS 保護プランの設定](#2-ddos-%E4%BF%9D%E8%AD%B7%E3%83%97%E3%83%A9%E3%83%B3%E3%81%AE%E8%A8%AD%E5%AE%9A)
...
4. \[**ネットワーク セキュリティ | Web アプリケーション ファイアウォール**\] 画面で、画面上部の \[+ 作成\] ボタンをクリックします
...
6. 画面右に \[**DDoS 保護の構成**\] ブレードが表示されるので、以下の項目を設定します
    ...
    ![パブリック IP アドレスへの DDoS 保護プランの適用](images/pip-apply-DDosProtection2.png)
...
7. この演習で作成した パブリック IP アドレス `handson-AG-PIP` で、DDoS 保護のメトリック アラートと、診断設定が有効になっていることを確認します。
    ...
    メトリック チャート上にあるボックスの \[**メトリック**\] ドロップダウン ボックスをクリックし、表示されたメトリック一覧の **Data Path Availability** 中に **DDoS** というサフィックスが付いたメトリックが存在することを確認します。
    ...
    * [**DDoS 攻撃を受けた IP アドレスの メトリック**](https://learn.microsoft.com/ja-jp/azure/ddos-protection/ddos-protection-features#metric-for-an-ip-address-under-ddos-attack)
...
この作業では、DDoS 保護プランを作成し、Application Gateway が使用するパブリック IP アドレス `handson-AG-PIP` に適用しました。

これにより、パブリック IP アドレスでは DDoS 保護のメトリックを検出できるようになり、同ログを Log Analytics ワークスペースに送信できるようになりました。

また、この環境では、Application Gateway WAF と組み合わせることで、[レイヤー 7 のアプリケーション レイヤーでの DDoS 保護](https://learn.microsoft.com/ja-jp/azure/web-application-firewall/shared/application-ddos-protection#azure-waf-with-azure-application-gateway)も提供します。


### メモ

今回の作業では、パブリック ネットワークからの攻撃を軽減するために、パブリック IP アドレスに DDoS 保護プランを適用しましたが、Azure Virtual Network (仮想ネットワーク) に DDoS 保護プランを適用することもできます。

![仮想ネットワークへの DDoS 保護プランの関連付け](images/vnet-jpeast-apply-DDosProtection.png)

詳細については、以下の記事を参照してください。

* [**仮想ネットワークの DDoS 保護を有効にする**](https://learn.microsoft.com/ja-jp/azure/ddos-protection/manage-ddos-protection#enable-ddos-protection-for-a-virtual-network)

<br>

## まとめ

この演習では、Application Gateway に WAF ポリシーを作成して適用し、パブリック IP アドレスに DDoS 保護プランを適用しました。

これにより、Application Gateway では WAF のメトリックを検出できるようになり、WAF ログを Log Analytics ワークスペースに送信できるようになりました。

また、パブリック IP アドレスでは DDoS 保護のメトリックを検出できるようになり、同ログを Log Analytics ワークスペースに送信できるようになりました。

<br>

---

👈　演習 6-1 : Application Gateway を介したアプリケーションの公開(HTTPS)(※作成中) 
