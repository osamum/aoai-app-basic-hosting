...
今回の作業では、パブリック IP アドレスに DDoS 保護プランを適用しましたが、Azure Virtual Network (仮想ネットワーク) に DDoS 保護プランを適用することもできます。

仮想ネットワーク全体に対して DDoS 保護を適用することで、仮想ネットワーク内のすべての Standard SKU のパブリック IP が保護されます。

複数のパブリック IP を持つ場合や、将来的に拡張する可能性がある場合は、仮想ネットワーク単位での保護が効率的です。

仮想ネットワーク保護は包括的で、複数リソースをまとめて保護したい場合に有効です。
