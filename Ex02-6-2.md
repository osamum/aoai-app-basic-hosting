...
2.  診断設定の追加の作成画面に遷移するので、同画面の各項目を以下のように設定します
    ...
    * **宛先の詳細**
    ...
    なお、Azure AI リソースの場合、ログ データがクエリや分析に**使用できるようになるまでに最大 2 時間かかる**ことがあります。そのため、診断設定を追加した直後にログをクエリしても、まだデータが存在しないのでしばらく待ちます。

3. 画面左のメニューの \[**監視**\] グループにある \[**ログ**\] をクリックします

```
AzureDiagnostics
| where Resource == "AOAI-20250606T014356Z" and Category == "RequestResponse"
| sort by TimeGenerated desc
```



AOAI_log_LogAnalytics.png

AOAI_log_queryResult.png

AOAI_New_DiagSetting.png
AOAI_Create_DiagSetting.png

Storage_Choose_DiagSetting.png
storage_Create_DiagSetting.png

https://learn.microsoft.com/ja-jp/azure/ai-services/diagnostic-logging#enable-diagnostic-log-collection


Azure AI 検索の診断ログを構成する
https://learn.microsoft.com/ja-jp/azure/search/search-monitor-enable-logging
