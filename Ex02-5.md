...
そのため既存の App Service で動作するアプリケーションの可用性ゾーンを有効にするには、新規に App Service プランを作成し、アプリケーションを移行する必要があります。ただし、Premium v2-4 または Isolated v2 プランの種類である必要があり、また、移行対象となる App Service と移行先のApp Serviceプランは同一リージョンである必要があります。
