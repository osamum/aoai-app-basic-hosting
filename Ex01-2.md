...
演習用アプリケーションを入手し、実行可能状態とします。

アプリケーションのプロジェクトの入手は GitHub 上にあるアプリケーションのリポジトリを直接クローンするのではなく、いったん受講者自身の GitHub アカウントにコピーしてから行います。

この設定は演習 3 の [GitHub Actions を使用した CI/CD 環境の構築](Ex03-1.md#2-github-actions-%E3%82%92%E4%BD%BF%E7%94%A8%E3%81%97%E3%81%9F-cicd-%E7%92%B0%E5%A2%83%E3%81%AE%E6%A7%8B%E7%AF%89) で必要になります。
...
* [**Node.js ランタイム**](https://nodejs.org/ja/download/)

* [**Git tool**](https://git-scm.com/downloads)

## 1. 演習用アプリケーションの入手

演習用アプリケーションのプロジェクトはテンプレート リポジトリとして作成されています。
...
5. `.env` ファイルを開き、[接続情報の取得](Ex01-1.md#%E6%8E%A5%E7%B6%9A%E6%83%85%E5%A0%B1%E3%81%AE%E5%8F%96%E5%BE%97)で取得した接続情報を使用して、以下の内容を参考に環境設定を行います。なお、ここまでの手順のとおり設定を行っているのであれば `AZURE_OPENAI_ENDPOINT` と `AZURE_OPENAI_API_KEY` の **2 つの値を設定するだけ**で、他の設定は設定値のままで問題ありません。
   
    ```env
    #Azure OpenAI Service リソースの接続情報
    AZURE_OPENAI_ENDPOINT=　Azure OpenAI Service リソースのエンドポイント URL
    AZURE_OPENAI_API_KEY=　Azure OpenAI Service リソースの API キー
    ...
    ```
    設定が完了したら、キーボードの \[**Ctrl + S**\] キーを押して保存します。

6. アプリケーションの依存関係をインストールします
   ...
   Visual Studio Code メニュー \[View\] - \[Terminal\] をクリックしてターミナルを開き、以下のコマンドを実行します。
    ...
    npm install
    ```
7. 依存関係のインストールが完了したら、以下のコマンドを実行してアプリケーションを起動します
...
8. ブラウザが起動し、アプリケーションのトップページが表示されることを確認します
...
>[!TIP]
>この演習で追加したコードを実行して 429 エラーが発生する場合は、以下の手順を参考に、言語モデルの**1 分あたりのトークン数レート制限**の値を引き上げてください。
>
>\[手順\]
> 1. [Azure AI Foundry](https://ai.azure.com/) 画面で対象の Azure OpenAI リソースを選択します
> 2. 画面左側のメニューから \[**デプロイ**\] をクリックし、対象の言語モデルのデプロイ名(`gpt-4o-mini`)をクリックしして、画面上部の \[**編集**\] ボタンをクリックします
>   <img src="images/lm_edit.png" alt="言語モデルの編集" width="500px">
>
> 3. 表示されたダイアログボックスの \[デプロイの詳細\] を展開し、\[**1 分あたりのトークン数レート制限**\] の値を最大まで引き上げて \[**保存して閉じる**\] ボタンをクリックします
...
次の準備作業では、Azure AI Search のインデックスを作成し、RAG を有効化します。
