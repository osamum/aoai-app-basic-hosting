...
(※2) セキュリティ上の観点から、インターネットに公開する場合は HTTPS を使用しないことは推奨されません。
...
無料マネージド証明書で十分であるため、外部の証明書を用意する必要はありません。
    ...
    もし、有効な独自ドメインがあり、証明書の用途が検証用であれば Microsoft ID プラットフォームの機能を使用して自己署名証明書を発行することもできます。手順については以下のドキュメントをご参照ください。

    * [自己署名公開証明書を作成してアプリケーションを認証する - **秘密キーがある公開証明書をエクスポートする**](https://learn.microsoft.com/ja-jp/entra/identity-platform/howto-create-self-signed-certificate#optional-export-your-public-certificate-with-its-private-key)


この演習で実行する作業は以下のとおりです。

1. [Key Vault への証明書のアップロード (※)](#1-key-vault-%E3%81%B8%E3%81%AE%E8%A8%BC%E6%98%8E%E3%81%AE%E3%82%A2%E3%83%83%E3%83%97%E3%83%AD%E3%83%BC%E3%83%89)
...
4. \[**Azure Key Vault からの証明書の選択**\] ブレードが表示されるので、以下のように各項目を設定します
    ...
    | キー コンテナー \*| *証明書をアップロードした Key Vault* |
    | 証明書 \* | *アップロードした証明書* |
